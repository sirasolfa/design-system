...
## 5. 아이콘에는 Figma 컴포넌트 설명이 붙어 있습니다 `[F]`

`get_design_context` 응답의 **Component descriptions** 항목으로 확인됐습니다.
아이콘마다 용도·사용처·프로퍼티 의미가 문장으로 적혀 있습니다.

예 — `application/settings` (`1212:10482`):

> 환경설정을 나타내는 톱니바퀴 아이콘입니다. 설정 화면 진입점에 사용합니다.
> Filled로 외곽선(false)·채움(true)을 선택합니다.

예 — `Decorate/Interaction/Normal` (`1171:8711`):

> - 인터랙션을 일반적으로 나타낼 때 사용합니다.
> - 요소 속 가장 위 레이어 상단에 올려 사용합니다.
> - 색을 바꿔 쓸 수 있습니다.

**이 설명들은 레이어 이름이나 베리언트 목록에는 나타나지 않습니다.**
`get_metadata` 로는 보이지 않고, 해당 아이콘을 쓰는 노드에 `get_design_context` 를 호출해야 나옵니다.
아이콘 200종 전체의 설명은 이번 조사에서 수집하지 않았습니다. `[?]`
...
| 7 | 아이콘 200종 각각의 Figma 컴포넌트 설명 (전량 미수집) |
